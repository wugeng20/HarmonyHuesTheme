...
- 文章类型：支持文章、图片两种类型（视频类型待开发）
- 文章编辑：内置文章内插入视频、网盘下载、提示框、折叠框、宫格图片功能
- 内置插件（免插件）：代码高亮（支持多种语言）、回复邮件提醒、友情链接、sitemap、表情包等等
- 特色独立页面：我的装备、明信片留言等等
...
- 【重要】更新前请务必备份主题文件、数据库、网站数据，防止更新失败导致数据丢失
- 【重要】更新主题之后，若网站CDN部署，请自行清理缓存，清除assets目录下所有静态文件缓存，防止新的静态文件不生效，导致页面显示异常
- 【重要】若修改主题代码，请自行备份，防止更新后丢失。先更新主题，如何再添加修改的部分，不要直接覆盖上更新后新主题文件，因为新主题文件可能有修改过的代码，直接覆盖会导致新代码丢失
