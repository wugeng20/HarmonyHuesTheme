 ...
 * 生成弹窗面板
 * @function
 * @param {string} id - 弹窗容器ID
 * @param {string} html - 弹窗内容HTML字符串
 * @param {string} [focusSelector] - 可选，弹窗内需要自动聚焦的元素选择器
 */
function createPanel(id, html, focusSelector) {
    $('body').append(
        `<div id="${id}">
      <div class="wmd-prompt-background"></div>
      <div class="wmd-prompt-dialog">${html}</div>
    </div>`
    );
    if (focusSelector) {
        $(`#${id} ${focusSelector}`)[0]?.select();
    }
}
...
window.onload = function () {
    $(document).ready(function () {
        if ($('#wmd-button-row').length > 0) {
            // 添加分割栏
            $('#wmd-more-button').after(
                `<li class="wmd-spacer wmd-spacer3" id="wmd-spacer5"></li>` +
...
function insertCustomButtons() {
    $('#wmd-spacer5').after(
        ...
        + '<li class="wmd-button" id="wmd-gridimg-button" title="宫格图片"><svg viewBox="0 0 1024 1024" p-id="8155" width="20" height="20"><path d="M606.8 917.6c-1.3-0.2-2.6-0.4-3.8-0.7 1.2 0.2 2.5 0.5 3.8 0.7zM606.8 398.5c-1.3 0.2-2.6 0.4-3.8 0.7 1.2-0.3 2.5-0.6 3.8-0.7zM610.7 918c-1.3-0.1-2.6-0.2-3.9-0.4 1.3 0.2 2.6 0.3 3.9 0.4zM610.7 398c-1.3 0.1-2.6 0.2-3.9 0.5 1.2-0.3 2.6-0.4 3.9-0.5zM879.9 883c0.8-0.2 1.6-0.5 2.3-0.8-0.7 0.4-1.5 0.6-2.3 0.8z" fill="#9b9b9b" opacity=".4" p-id="11976"></path><path d="M562.9 256.4V149.8c0-28.6 23.3-51.9 51.9-51.9h260.5c28.6 0 51.9 23.3 51.9 51.9v106.6c0 28.6-23.3 52-51.9 52H614.8c-28.6 0-51.9-23.3-51.9-52zM462.8 151.5v416.3c0 27.8-22 50.6-49.4 51.9-0.9 0-1.7 0.1-2.6 0.1H148.9c-28.2-0.6-50.9-23.7-50.9-52V151.5c0-28.7 23.3-52 52-52h260.8c0.9 0 1.7 0 2.6 0.1 27.5 1.3 49.4 24.1 49.4 51.9zM462.7 765.4v106.5c0 28.7-23.3 52-52 52H150.3c-28.7 0-52-23.3-52-52V765.4c0-28.7 23.3-52 52-52h260.4c28.7 0 52 23.3 52 52zM875.6 397.8H614.7c-0.9 0-1.8 0-2.7 0.1-0.5 0-0.9 0-1.3 0.1-1.3 0.1-2.6 0.2-3.9 0.5-1.3 0.2-2.6 0.4-3.8 0.7-0.7 0.2-1.5 0.4-2.2 0.5-1.8 0.5-3.5 1.1-5.2 1.8-0.3 0.1-0.7 0.3-1 0.4-0.8 0.3-1.5 0.7-2.2 1-1.5 0.7-3 1.5-4.4 2.4-15.1 9.1-25.1 25.7-25.1 44.5v416.3c0 18.8 10.1 35.4 25.1 44.5 1.4 0.9 2.9 1.7 4.4 2.4 0.7 0.4 1.5 0.7 2.2 1 0.3 0.1 0.7 0.3 1 0.4 1.7 0.7 3.5 1.3 5.2 1.8 0.7 0.2 1.4 0.4 2.2 0.5 1.3 0.3 2.5 0.5 3.8 0.7 1.3 0.2 2.6 0.3 3.9 0.4 0.4 0 0.9 0.1 1.3 0.1 0.9 0 1.8 0.1 2.7 0.1h260.8c3.7 0 7.4-0.4 10.9-1.2 0.7-0.1 1.4-0.3 2-0.5 2.1-0.5 4.1-1.2 6-2 0.8-0.3 1.6-0.6 2.3-1 18.1-8.1 30.7-26.3 30.7-47.4v-416c0.1-28.7-23.2-52-51.8-52.1zM608.1 882.3c1.8 0.7 3.7 1.1 5.6 1.2-1.9 0-3.8-0.5-5.6-1.2z" fill="#9b9b9b" p-id="11977"></path><path d="M613.8 883.6c-2-0.1-3.9-0.5-5.6-1.2 1.7 0.6 3.6 1.1 5.6 1.2zM879.9 883c0.8-0.2 1.6-0.5 2.3-0.8-0.7 0.4-1.5 0.6-2.3 0.8z" fill="#9b9b9b" p-id="11978"></path><path d="M608.1 882.3c1.8 0.7 3.7 1.2 5.6 1.2-1.9 0-3.8-0.5-5.6-1.2z" fill="#9b9b9b" p-id="11979"></path></svg></li>'
    );
    $('#wmd-spacer6').after(
...
function bindButtonEvents() {
    const panelMap = {
        '#wmd-t-button': showTextStylePanel,
        '#wmd-video-button': showVideoPanel,
        '#wmd-msgpro-button': showMessagePromptPanel,
        '#wmd-tolinks-button': showLinkCardPanel,
        '#wmd-cloud-button': showCloudPanel,
        '#wmd-fold-button': showFoldPanel,
        '#wmd-friendshiplink-button': showFriendshipLinkPanel,
        '#wmd-equipment-button': showMeEquipmentPanel,
        '#wmd-gridimg-button': showGridImgPanel
    };
    Object.entries(panelMap).forEach(([selector, fn]) => {
        $(document).on('click', selector, fn);
    });

    // 确认/取消事件
    $(document)
        .on('click', '#video_ok', handleVideoConfirm)
        .on('click', '#text_ok', handleTextStyleConfirm)
        .on('click', '#wmd-indentation-button', indentationConfirm)
        .on('click', '#mes_prompt_ok', handleMesPromptConfirm)
        .on('click', '#tolinks_ok', handleLinkCardConfirm)
        .on('click', '#cloud_ok', handleCloudConfirm)
        .on('click', '#fold_ok', handleFoldConfirm)
        .on('click', '#friendship_ok', handleFriendshipLinkConfirm)
        .on('click', '#equipment_ok', handleMeEquipmentConfirm)
        .on('click', '#gridimg_ok', handleGridImageConfirm)
        .on('click', '#text_cancel', () => closePanel('#TPanel'))
        .on('click', '#video_cancel', () => closePanel('#videoPanel'))
        .on('click', '#mes_prompt_cancel', () => closePanel('#messagePromptPanel'))
        .on('click', '#tolinks_cancel', () => closePanel('#linkCardPanel'))
        .on('click', '#cloud_cancel', () => closePanel('#cloudPanel'))
        .on('click', '#fold_cancel', () => closePanel('#foldPanel'))
        .on('click', '#friendship_cancel', () => closePanel('#friendshipLinkPanel'))
        .on('click', '#equipment_cancel', () => closePanel('#meEquipmentPanel'))
        .on('click', '#gridimg_cancel', () => closePanel('#gridImgPanel'));
}
...
function showMessagePromptPanel() {
    createPanel('messagePromptPanel',
        '<div>' +
        '<p><b>插入消息提示框</b></p>' +
        '<p><labe>提示框样式</labe></p>' +
...
function showFriendshipLinkPanel() {
    createPanel('friendshipLinkPanel',
        '<p><b>添加友情链接</b></p>' +
...
/* 宫格grid图片面板 */
function showGridImgPanel() {
    createPanel('gridImgPanel',
        '<div>' +
        '<p><b>宫格图片</b></p>' +
        ...
        '<button type="button" class="btn btn-s" id="gridimg_cancel">取消</button>' +
        '</form>'
    );
}
...
/* 处理缩进事件 */
function indentationConfirm() {
    const myField = document.getElementById('text');
    const insertContent = "　";
    console.log("测试测试");

    insertContentToTextArea(myField, insertContent, '');
}
 ...
 * 处理链接卡片的确认事件
 */
function handleLinkCardConfirm() {
    const myField = document.getElementById('text');
...
/**
 * 关闭弹窗面板
 * @param {string} panelId - 需要关闭的弹窗容器选择器
 */
...
/**
 * 向编辑器插入内容
 * @param {HTMLTextAreaElement} textarea - 编辑器文本域
 * @param {string} content - 需要插入的内容
 * @param {string} modelId - 弹窗面板ID
 */
