...
function parseLinksShortcode($content) {
    ...
    foreach ($matches as $match) {
        ...
        // 优化属性解析正则，支持带空格的引号值和未加引号的值
        preg_match_all('/(\w+)=("([^"]*)"|\'([^\']*)\'|([^\s]+))/', $match[1], $attrs, PREG_SET_ORDER);

        foreach ($attrs as $attr) {
            $key = strtolower($attr[1]); // 统一转为小写
            // 安全提取值，避免 Undefined array key 错误
            $value = $attr[3] ?? ($attr[4] ?? ($attr[5] ?? ''));

            if (array_key_exists($key, $defaults)) {
                $link[$key] = htmlspecialchars_decode($value); // 处理可能的 HTML 实体
            }
        }

        // 如果 group 为空，则使用默认值 "网上邻居"
        if (empty($link['group'])) {
            $link['group'] = '网上邻居';
        }
    ...
    <div class="links-head my-4 text-center">
      <div class="links-title font-weight-bold">
        <p class="text-shadow-style">
          <?php $this->authorId ? $this->author->screenName() : $this->user->screenName(); ?>のTA的朋友们</p>
      </div>
    </div>
    <!-- 友邻列表srt -->
    <?php foreach ($groupedLinks as $groupName => $links): ?>
    <?php $linksCount = count($links);if ($linksCount === 1 && isset($links[0]['status']) && $links[0]['status'] === '隐藏'): continue;endif; ?>
    <section class="links-group my-4">
      ...
      <div class="links-list row no-gutters">
        ...
        <div class="col-6 col-lg-3 d-flex flex-column align-self-stretch">
          <div class="links-card flex-fill p-2 m-1"><a class="d-flex flex-direction align-items-center"
              href="<?php echo $link['url']; ?>" target="_blank" title="<?php echo $link['title']; ?>">
        ...
        <div class="links-checkboxs ml-2">
          <p>
            <label class="checkbox"><input type="checkbox" class="links-checkbox">&nbsp;我已添加：<b><a
                  href="<?php $this->options->siteUrl(); ?>"><?php $this->options->title()?></a></b>
              博客的友情链接</label>
          </p>
          <p>
